...
const CardHighLevelRiskService = () => {
    let cards: any[]
    ...
    const cardHighLevelRiskList = [];
  
    for (const key in data) {
      if (Object.hasOwnProperty.call(data, key)) {
        const element = data[key];
        if (element.riskLevel == 'High' || element.riskLevel == 'Medium'){
          cardHighLevelRiskList.push(
            <Card
              image={element.image}
              risk={element.risk}
              title={element.title}
    ...
    return (
      <div>
        {cardHighLevelRiskList}
      </div>
    )
  }
  
  export default CardHighLevelRiskService;
